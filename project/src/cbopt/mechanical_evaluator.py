"""Nonlinear compression mechanics for Paper A size--shape geometries.

The evaluator applies Paper A's displacement-controlled, periodic unit-cell
protocol to one NURBS period.  It deliberately
keeps the finite-element response separate from the geometric board
homogenization: the corrugated medium is compressed between rigid horizontal
surfaces, while the two liner contributions enter only the material-fraction
bookkeeping used by Paper A.
"""
...
from .evaluator import evaluate_design, nurbs_profile
...
PROTOCOL_VERSION = "paper-a-periodic-c1-attached-contact-signed-reactions-v9"
...
def solve_compression_path(
    ...
    *,
    ...
    position = np.asarray(position, dtype=float)
    height = float(position[8])
    thickness = float(position[9])
    nodes, discretized_arc = paper_a_profile_nodes(position, elements_per_wavelength)
